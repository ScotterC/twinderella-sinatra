...
class App < Sinatra::Base
  ...
  get '/auth/:provider/callback' do
    ...
    unless User.find_by_uid(omniauth[:uid])
      ...
      # usernameescaped = "scott%40artsicle.com"
      # responsed = RestClient.put("https://#{usernameescaped}:#{auth_params['password']}@posterous.com/api/2/sites/#{site_id}/profile/image", 
      #   {"file" => File.new(tempfile.path, "rb"), 'api_token' => auth_params['api_token']})

      # uri = URI.parse("http://posterous.com/api/2/sites/#{site_id}/profile/image")

      # http = Net::HTTP.new(uri.host, uri.port)
      # req = Net::HTTP::Put.new(uri.request_uri)
      # req.basic_auth(auth_params['username'], auth_params['password'])
      # req.set_form_data({ "file" => File.new(tempfile.path, 'rb'), 'api_token' => auth_params['api_token'] })
      # response = http.request(req)

    #   res = ""
    #   url = URI.parse("http://posterous.com/api/2/sites/#{site_id}/profile/image")
    #   File.open(tempfile.path) do |jpg|
    #     req = Net::HTTP::Put::Multipart.new url.path,
    #       "file" => UploadIO.new(jpg, "image/jpeg", "image.jpg")
    #     req.basic_auth(auth_params['username'], auth_params['password'])
    #     req.set_form_data({'api_token' => auth_params['api_token'] })
