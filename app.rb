...
class App < Sinatra::Base
  ...
  get '/auth/:provider/callback' do
    ...
    unless User.find_by_uid(omniauth[:uid])
      ...
      # tempfile = Tempfile.new('profile.png')
      ...
      # RestClient.put("http://#{auth_params['username']}:#{auth_params['password']}@posterous.com/api/2/sites/#{site_id}/profile/image", 
      #   {"file" => File.new(tempfile.path), 'api_token' => auth_params['api_token']})
      ...
      # req.basic_auth(auth_params['username'], auth_params['password'])
      # req.set_form_data({ "file" => File.new(tempfile.path), 'api_token' => auth_params['api_token'] })
      # response = http.request(req)
